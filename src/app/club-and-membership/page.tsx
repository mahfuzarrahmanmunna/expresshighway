...
const STEPS = [
  {
    num: "01",
    title: "Enquire",
    desc: "Submit the membership form or call our team.",
  },
  {
    num: "02",
    title: "Verify",
    desc: "Our team confirms your details and preferred tier.",
  },
  {
    num: "03",
    title: "Activate",
    desc: "Receive your membership card and start using the Club & Lounge immediately.",
  },
];
...
export default function ClubAndMembershipPage() {
  ...
  useGSAP(
    () => {
      ...
      // Generic Stagger Reveal for Cards/Items
    ...
    <main ref={containerRef} className="relative w-full bg-[#F7F6F2] text-[#0c0b0b] overflow-x-hidden font-[family-name:var(--font-sans)]">
      ...
      <section className="relative w-full max-w-7xl mx-auto px-6 lg:px-8 py-24 md:py-32 z-10">
        <div className="flex flex-col items-center text-center mb-20">
          <span className="reveal-item block text-[10px] uppercase tracking-[0.4em] text-[#C5A572] font-bold mb-6">
            Why Join
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-[#0c0b0b]/10 border border-[#0c0b0b]/10">
          {BENEFITS.map((benefit, i) => (
            <div 
              key={i} 
              className="reveal-item group relative bg-[#F7F6F2] hover:bg-white transition-colors duration-500 p-8 md:p-10 flex flex-col gap-6 min-h-[220px] overflow-hidden"
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="font-[family-name:var(--font-playfair)] text-2xl text-[#0c0b0b]/10 transition-colors duration-500 group-hover:text-[#C5A572]/20">
                  {String(i + 1).padStart(2, "0")}
                </span>
              </div>
              <p className="text-sm md:text-base font-light text-[#0c0b0b]/70 leading-[1.8]">
                {benefit}
              </p>
            </div>
          ))}
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10 border border-white/10">
            {STEPS.map((step, i) => (
              <div 
                key={step.num} 
                className="reveal-item group relative bg-[#080808] hover:bg-[#111111] transition-colors duration-500 p-10 md:p-12 min-h-[300px] flex flex-col justify-between overflow-hidden"
              >
                {/* Ghost Architectural Index */}
                <span className="absolute top-4 right-4 text-[100px] leading-none font-[family-name:var(--font-playfair)] text-white/[0.03] pointer-events-none transition-colors duration-500 group-hover:text-[#C5A572]/10">
                  {step.num}
                </span>

                <div>
                  <span className="text-[10px] uppercase tracking-[0.3em] text-[#C5A572] font-bold mb-4 block">Step {step.num}</span>
                  <h3 className="font-[family-name:var(--font-playfair)] text-3xl md:text-4xl font-light text-white mb-4 tracking-tight">
                    {step.title}
                  </h3>
                  <div className="w-10 h-px bg-[#C5A572] mb-6 transition-all duration-500 group-hover:w-16"></div>
                </div>
                
                <p className="text-sm md:text-base font-light text-white/50 leading-[1.8] relative z-10">
                  {step.desc}
                </p>
              </div>
            ))}
          ...
          {!isSubmitted ? (
            <form onSubmit={handleSubmit} className="reveal-item relative bg-white border border-[#0c0b0b]/10 p-8 md:p-12 lg:p-16 overflow-hidden shadow-xl">
              ...
              <div className="relative z-10 flex flex-col gap-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <select 
                      ...
                      className="w-full bg-[#F7F6F2] border border-[#0c0b0b]/15 px-4 py-[14px] text-base focus:outline-none focus:border-[#C5A572] focus:bg-white transition-all duration-500 text-[#0c0b0b] font-medium appearance-none cursor-pointer"
                    >
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-4">
                  <p className="text-[10px] uppercase tracking-[0.2em] text-[#0c0b0b]/40 font-light text-left md:max-w-xs">
                    Note: This is a lead-capture form, not a payment checkout - membership is confirmed via the Sampan team.
                  </p>
                  <button 
                    type="submit" 
                    className="group relative inline-flex items-center justify-center gap-3 px-10 py-5 bg-[#0c0b0b] text-white text-[10px] uppercase tracking-[0.35em] font-bold overflow-hidden cursor-pointer hover:bg-[#C5A572] hover:text-[#0c0b0b] transition-colors duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] w-full md:w-auto"
                  >
            ...
            <div className="reveal-item relative bg-white border border-[#0c0b0b]/10 p-12 md:p-20 flex flex-col items-center justify-center text-center min-h-[400px] overflow-hidden shadow-xl">
              ...
              <p className="text-sm md:text-base font-light text-[#0c0b0b]/60 leading-[1.8] max-w-md mb-10">
                Thank you for your interest in the Express Highway Inn Club &amp; Lounge. Our concierge team will contact you shortly to complete the process.
              </p>
            </div>
          )}
        </div>
